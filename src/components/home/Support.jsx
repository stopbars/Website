import { Check, Copy, MessageSquareMore, FileText, AlertCircle, ExternalLink } from 'lucide-react';
    ...
    <section className="py-32 relative overflow-hidden" id="support">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="group relative p-8 rounded-2xl bg-linear-to-br from-zinc-900 to-zinc-900/50 border border-zinc-800 hover:border-zinc-700 transition-colors duration-300 flex flex-col">
            <div className="relative flex flex-col flex-1">
              <div className="w-14 h-14 mb-6 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center">
                <AlertCircle className="w-7 h-7 text-amber-400" />
              </div>

              <h3 className="text-xl font-semibold mb-3">Report an Issue</h3>
              <p className="text-zinc-400 text-sm mb-6 leading-relaxed flex-1">
                Found a bug? Report it through our Discord or GitHub with your log files to help us
                investigate.
              </p>

              {/* Log Path Section */}
              <div className="flex items-center justify-between gap-2 p-2.5 bg-zinc-800/50 rounded-lg border border-zinc-700/50 mt-auto">
                <code className="font-mono text-emerald-400 text-xs truncate">
                  %localappdata%/BARS
                </code>
                <button
                  onClick={handleCopyPath}
                  className="shrink-0 p-1.5 text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-md transition-colors cursor-pointer"
                  aria-label={
                    copiedPath ? 'Path copied to clipboard' : 'Copy log file path to clipboard'
                  }
                >
                  {copiedPath ? (
          ...
          <div className="group relative p-8 rounded-2xl bg-linear-to-br from-zinc-900 to-zinc-900/50 border border-zinc-800 hover:border-zinc-700 transition-colors duration-300 flex flex-col">
            <div className="relative flex flex-col flex-1">
              <div className="w-14 h-14 mb-6 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center">
                <MessageSquareMore className="w-7 h-7 text-indigo-400" />
              </div>
