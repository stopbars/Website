...
export function contributionSimulatorOptions(testedSimulator) {
  if (testedSimulator === 'xplane') return ['xplane'];
  if (['msfs2020', 'msfs2024'].includes(testedSimulator)) return ['msfs2024', 'msfs2020'];
  return [];
}
...
export function contributionProofError(proof, simulator) {
  ...
  if (!contributionSimulatorOptions(proof.testedSimulator).includes(simulator)) {
    return 'The simulator changed after testing. Return to Test and prepare this draft for the selected simulator again.';
  }
  return '';
}

export async function prepareContributionSubmissionProof(proof, simulator, xml, icao, request = fetch) {
  const error = contributionProofError(proof, simulator);
  if (error) throw new Error(error);
  if (proof.testedSimulator === simulator) return proof;

  const formData = new FormData();
  formData.append('xmlFile', new Blob([xml], { type: 'application/xml' }), `${icao}-Draft.xml`);
  formData.append('icao', icao);
  formData.append('simulator', simulator);
  const response = await request('https://v2.stopbars.com/supports/generate', {
    method: 'POST',
    body: formData,
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Unable to prepare the selected MSFS version.');
  if (!data.token || data.generationHash !== proof.generationHash) {
    throw new Error('The draft no longer matches the tested version. Return to Test and prepare it again.');
  }
  return { generationToken: data.token, generationHash: data.generationHash, testedSimulator: simulator };
}
