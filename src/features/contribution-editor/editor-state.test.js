...
test('additive manual removal retains the automatic record with a distinct id', () => {
  const coordinates = [[115, -32], [115.001, -32], [115.001, -32.001], [115, -32]];
  const automatic = { id: 'same-geometry', origin: 'msfs-auto', sourceIds: ['row-a'], coordinates };
  const state = createEditorState({ ...document, removals: [automatic] });
  const updated = editorReducer(state, {
    type: 'replace-msfs-edited-removals', preserveAutomatic: true,
    sourceIds: ['row-a'], removalIds: ['same-geometry'],
    removals: [{ id: 'same-geometry', sourceIds: ['row-a'], coordinates }],
  });
  assert.equal(updated.present.removals.length, 2);
  assert.deepEqual(updated.present.removals[0], state.present.removals[0]);
  assert.equal(updated.present.removals[1].id, 'msfs-manual:same-geometry');
  assert.equal(updated.present.removals[1].origin, 'msfs-manual');
});
...
test('automatic refresh replaces old automatic polygons and preserves manual decisions', () => {
  const state = createEditorState({
    ...document,
    removals: [
      { id: 'old-auto', origin: 'msfs-auto', sourceIds: ['old'], coordinates: [] },
      { id: 'manual-remove', origin: 'msfs-manual', sourceIds: ['manual'], coordinates: [], selections: [{ sourceId: 'manual' }] },
      { id: 'saved-keep', origin: 'imported', importedOrigin: 'msfs-manual', sourceIds: ['keep'], coordinates: [], keepSelections: [{ sourceId: 'keep' }] },
    ],
  });
  const originalManual = state.present.removals.filter(removal => removal.id !== 'old-auto');
  const updated = editorReducer(state, {
    type: 'refresh-automatic-msfs-removals',
    removalIds: ['old-auto', 'manual-remove', 'saved-keep'],
    sourceIds: ['fresh', 'manual', 'keep'],
    removals: [{ id: 'new-auto', sourceIds: ['fresh'], coordinates: [] }],
  });
  assert.deepEqual(updated.present.removals.filter(removal => removal.id !== 'new-auto'), originalManual);
  assert.equal(updated.present.removals.at(-1).origin, 'msfs-auto');
  assert.ok(!updated.present.removals.some(removal => removal.id === 'old-auto'));
});
